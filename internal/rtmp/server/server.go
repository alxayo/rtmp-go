...
func (c *Config) applyDefaults() {
    if c.ListenAddr == "" { c.ListenAddr = ":1935" }
    if c.ChunkSize == 0 { c.ChunkSize = 4096 }            // matches control burst constant
    if c.WindowAckSize == 0 { c.WindowAckSize = 2_500_000 } // matches control burst
    if c.RecordDir == "" { c.RecordDir = "recordings" }
...
func (s *Server) Start() error {
    if s == nil { return errors.New("nil server") }
    s.mu.Lock()
    if s.l != nil {
        s.mu.Unlock()
...
func (s *Server) acceptLoop() {
    ...
    for {
        ...
        if err != nil {
            ...
            s.mu.RLock(); closing := s.closing; s.mu.RUnlock()
            if ne, ok := err.(net.Error); ok && ne.Timeout() { continue }
            if closing || errors.Is(err, net.ErrClosed) { return }
        ...
        s.mu.Lock(); s.conns[c.ID()] = c; s.mu.Unlock()
...
func (s *Server) Stop() error {
    if s == nil { return errors.New("nil server") }
    s.mu.Lock()
    if s.l == nil { s.mu.Unlock(); return nil }
    s.closing = true
    l := s.l
    s.l = nil
    s.mu.Unlock()
...
func (s *Server) Addr() net.Addr {
    s.mu.RLock(); defer s.mu.RUnlock(); if s.l == nil { return nil }; return s.l.Addr()
}

// ConnectionCount returns current number of tracked active connections.
func (s *Server) ConnectionCount() int {
    s.mu.RLock(); defer s.mu.RUnlock(); return len(s.conns)
}
...
func (s *singleConnListener) Accept() (net.Conn, error) { if s.conn == nil { return nil, errors.New("no conn") }; c := s.conn; s.conn = nil; return c, nil }
func (s *singleConnListener) Close() error              { if s.conn != nil { _ = s.conn.Close() }; return nil }
func (s *singleConnListener) Addr() net.Addr            { if s.conn != nil { return s.conn.LocalAddr() }; return &net.TCPAddr{} }
