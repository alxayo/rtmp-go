// Shell Hook
// ==========
// Runs a shell script when an RTMP event occurs. Event data is passed to
// the script as environment variables (RTMP_EVENT_TYPE, RTMP_STREAM_KEY, etc.).
// Optionally, the full event can be piped as JSON to the script's stdin.
package hooks
...
// ShellHook runs a shell script when an event occurs.
type ShellHook struct {
	id       string        // unique identifier for this hook
	command  string        // shell binary to execute (default: /bin/bash)
	args     []string      // arguments: typically the script path
	env      []string      // additional environment variables
	passJSON bool          // if true, pipe JSON event data to stdin
	timeout  time.Duration // maximum execution time before kill
}
