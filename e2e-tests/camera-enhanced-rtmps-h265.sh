...
#   TLS (RTMPS). Combines Enhanced RTMP H.265 codec negotiation with TLS
#   transport security. The server records the stream to an MP4 file.
#   After publishing, the recording is validated to contain HEVC video
#   and be fully decodable, and the TLS connection is verified in logs.
#
# EXPECTED RESULT:
...
#   - FFmpeg with libx265, aac, and TLS support
#   - openssl CLI (for cert generation)
#   - Go 1.21+
#
# USAGE:
#   ./e2e-tests/camera-enhanced-rtmps-h265.sh
# ============================================================================
set -euo pipefail
...
TEST_NAME="camera-enhanced-rtmps-h265"
...
STREAM_URL="rtmps://localhost:${TLS_PORT}/live/camera-secure-h265?tls_verify=0"

log_step "Publishing camera via Enhanced RTMPS H.265 (5s)..."
set +e
...
assert_log_contains "$SERVER_LOG" "\"tls\":true" "TLS connection confirmed"

# Find recording (H.265 → MP4, fallback to FLV)
RECORD_FILE=""
for f in "$RECORD_DIR"/live_camera-secure-h265_*.mp4 "$RECORD_DIR"/live_camera-secure-h265_*.flv; do
