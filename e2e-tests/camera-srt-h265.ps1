...
#   audio. Recording is enabled, and the test verifies the resulting file
#   (MP4 for H.265) contains valid HEVC video that can be decoded.
#
# EXPECTED RESULT:
#   - Server accepts SRT camera stream, recording created with HEVC, no panics
# ============================================================================
. "$PSScriptRoot\_lib.ps1"

$TestName = "camera-srt-h265"
...
Write-Host "$(Get-Date -Format 'HH:mm:ss') -> Publishing camera feed via SRT H.265 (5s)..." -ForegroundColor Blue
...
$proc = Start-Process -FilePath "ffmpeg" -ArgumentList @(
    ...
    "-c:a", "aac", "-b:a", "64k",
    "-t", "5", "-f", "mpegts",
    "srt://localhost:${SrtPort}?streamid=publish:live/camera-h265&latency=200000"
...
# H.265 recordings should be MP4; fall back to FLV
$recording = Get-ChildItem -Path $recordDir -Include "*.mp4","*.flv" -Recurse | Select-Object -First 1
if (-not $recording) {
    Fail-Check "H.265 recording file created" "No recording file found in $recordDir"
} else {
