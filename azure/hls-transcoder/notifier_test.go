...
func TestSegmentNotifier_ScanDir(t *testing.T) {
	...
	os.WriteFile(filepath.Join(streamDir, "seg_00001.ts"), []byte("ts-data"), 0o644)
	os.WriteFile(filepath.Join(streamDir, "seg_00002.ts"), []byte("ts-data"), 0o644)

	// First scan — should detect all 4 files
	...
	if count != 4 {
		t.Fatalf("expected 4 events after first scan, got %d", count)
	}

	// Second scan with no changes — should fire 0 new events
	n.scanDir(t.Context(), "live_test", dir, seen, playlistMods)
	...
	if count != 4 {
		t.Fatalf("expected still 4 events after second scan (no changes), got %d", count)
	}
	...
	// Add a new segment — should fire 1 event
	os.WriteFile(filepath.Join(streamDir, "seg_00003.ts"), []byte("ts-data"), 0o644)
	...
	if count != 6 {
		t.Fatalf("expected 6 events after new segment, got %d", count)
	}
}
...
func TestSegmentNotifier_EventFormat(t *testing.T) {
	...
	os.WriteFile(segFile, []byte("data"), 0o644)
...
func TestSegmentNotifier_WebhookError(t *testing.T) {
	...
	dir := t.TempDir()
	os.WriteFile(filepath.Join(dir, "seg_00001.ts"), []byte("data"), 0o644)
	...
	if _, ok := seen[filepath.Join(dir, "seg_00001.ts")]; !ok {
		t.Error("expected segment to be marked as seen even after webhook error")
	}
}
...
func TestSegmentNotifier_IgnoresNonHLSFiles(t *testing.T) {
	...
	dir := t.TempDir()
	// HLS files
	os.WriteFile(filepath.Join(dir, "seg_00001.ts"), []byte("ts"), 0o644)
	...
	if eventCount != 2 {
		t.Errorf("expected 2 events (ts + m3u8), got %d", eventCount)
	}
}
